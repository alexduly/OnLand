    ...
    pub fn load_polys(filename: &str) -> HashMap<String, GeometryCollection> {
        ...
        let mut reader: shapefile::Reader<
            std::io::BufReader<std::fs::File>,
            std::io::BufReader<std::fs::File>,
        > = shapefile::Reader::from_path(filename).expect("something went wrong");
        ...
        for result in reader.iter_shapes_and_records() {
            match result {
                Ok((shape, record)) => {
                    ...
                    let geo_polygon =
                        geo::Geometry::<f64>::try_from(shape).expect("didnt work - change later");
